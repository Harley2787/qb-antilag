...
AddEventHandler("2step:Anti-lag", function()
	for x,v in pairs(Config.TwoStepCars) do
	...
	if carAdded == true and antilag == false then	
		antilag = true
		QBCore.Functions.Notify("Anti-Lag Has Been Enabled!", "success")
	elseif antilag == true and carAdded == true then
		antilag = false
		QBCore.Functions.Notify("Anti-Lag Has Been Disabled!", "success")
	else
		antilag = false
		QBCore.Functions.Notify("Your Vehicle Does Not Have Anti-Lag Installed!", "error")
	end
    ...
    while true do
		...
		if IsControlPressed(1, Config.twoStepControl) then
			if IsPedInAnyVehicle(ped) then
				...
				if GetPedInVehicleSeat(pedVehicle, -1) == ped and antilag == true then
					local vehicleModel = GetEntityModel(GetVehiclePedIsIn(GetPlayerPed(-1)))
